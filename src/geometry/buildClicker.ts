...
export function buildClicker(
  ...
  if (params.baseShape === 'outline') {
    ...
    // "Size" (capWidthMm) is the CAP's longest side — the pressable top face itself.
    // We size the shape to that dimension directly and shrink the IMAGE to sit inside
    // it (NOT the reverse — growing the shape to circumscribe the image inflated a round
    // cap by up to √2× the requested Size). The body/bezel then grows outward from the
    // cap by the well tolerance + border, and that larger outer footprint is surfaced
    // separately as the "total base size" readout under the slider. Each generator is
    // linear in rr, so read the unit shape's bbox once and divide. Floor the cap at
    // minCap so a very small Size still clears the switch column (below that, the
    // switch — not Size — sets the size).
    const unit = genShape(1);
    const ub = unit.bounds();
    const unitLongest = Math.max(ub.max[0] - ub.min[0], ub.max[1] - ub.min[1]) || 2;
    const capLongest = Math.max(params.capWidthMm, minCap);
